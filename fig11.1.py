...
def generate_points(has_d_seed_trees=False,
                    has_aes_prg=False,
                    accel=True, slack=0):
    x = []
    y = []
    ...
    for N in N_range :
        for ell in [(slack+1)]:
            ...
            try:
                ggm = TCitH_GGM(N, ell, slack, masking_order, tau=None, mpc_protocol=mpc, kappa=kappa, \
                        has_d_seed_trees=has_d_seed_trees, has_aes_prg=has_aes_prg, accel=accel)
            except ValueError:
                continue
            tau = ggm.tau
            size = ggm.get_size()
            time, _, _ = ggm.get_performance_index(keccak_perf, aes_perf, isw_perf, refresh_perf, accel=accel)
...
x0, y0, z0 = generate_points(False, False, True, 0)
x1, y1, z1 = generate_points(True, False, True, 0)
x2, y2, z2 = generate_points(False, False, True, masking_order)
x3, y3, z3 = generate_points(True, False, True, (masking_order-1)//2)

ax.scatter(x0,z0,label="No tweak", s=dot_size, c=colors_scatter[2], marker=filled_markers[0])
ax.scatter(x1,z1,label="Parallel trees", s=dot_size, c=colors_scatter[3], marker=filled_markers[1])
ax.scatter(x2,z2,label="Slack (full)", s=dot_size, c=colors_scatter[4], marker=filled_markers[2])
ax.scatter(x3,z3,label="Slack (half) + tweak", s=dot_size, c=colors_scatter[5], marker=filled_markers[3])
...
ax.scatter(x0_pareto,z0_pareto, s=dot_size+5, marker=filled_markers[0], alpha=.7, facecolors='none', edgecolors='black')#color='black')
ax.scatter(x1_pareto,z1_pareto, s=dot_size+5, marker=filled_markers[1], alpha=.7, facecolors='none', edgecolors='black')
ax.scatter(x2_pareto,z2_pareto, s=dot_size+5, marker=filled_markers[2], alpha=.7, facecolors='none', edgecolors='black')
ax.scatter(x3_pareto,z3_pareto, s=dot_size+5, marker=filled_markers[3], alpha=.7, facecolors='none', edgecolors='black')
...
plt.title("TCitH-GGM masked signature, $N$ from "+str(N_min)+" to "+str(N)+", $d = "+str(masking_order)+"$")
...
plt.yscale("log")
plt.savefig("img/tcith-ggm-pc-comparison-"+str(masking_order+1)+"-shares.pdf", format="pdf", bbox_inches='tight')
plt.show()
#####################################################################